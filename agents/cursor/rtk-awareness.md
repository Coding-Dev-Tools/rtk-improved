# RTK Token Optimization for Cursor
...
before execution (`rtk hook cursor`, registered in `~/.cursor/hooks.json`). The
hook provides `updated_input` — the rewritten command replaces yours before it
runs. This is a convenience, not a mandate: you control when fidelity matters.
